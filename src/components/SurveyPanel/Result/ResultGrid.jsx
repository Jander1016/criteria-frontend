...
class ResultGrid extends React.Component {
  toggleSelectAll() {
    const { onSelectAll, onUnSelectAll, grid: { results } } = this.props;
  ...
  render() {
    const { onClickRow, onSelectRow, rowSelects, allSelected, onDeleteRow, surveyId } = this.props;
    const { columns, results } = this.props.grid;
    const anySelected = Object.keys(rowSelects).some(k => rowSelects[k]);
          ...
          <table className="table table-condensed table-hover table-bordered">
            ...
            <tbody>
              {results.map((result, index) => (
                <tr key={result._id} onClick={() => onClickRow(result, index)}>
